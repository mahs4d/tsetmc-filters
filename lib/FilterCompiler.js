...
class FilterCompiler {
    static async compile(entryFileAddress, outputFileAddress) {
        ...
        const prependContent = Fs.readFileSync(Path.join(__dirname, '../data/filtermeta/filter.prepend.js'));
        await PrependFile(outputFileAddress, prependContent);

        const appendContent = Fs.readFileSync(Path.join(__dirname, '../data/filtermeta/filter.append.js'));
